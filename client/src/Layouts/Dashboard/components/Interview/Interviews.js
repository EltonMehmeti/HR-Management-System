...
const Interviews = () => {
  ...
  const handleStatusChange = (newStatus, interview) => {
    setStatus(newStatus);
    editInterviewStatus(newStatus, interview.id);
    if (newStatus === 'job_offer') {
    ...
    >
      <span className="absolute top-2 right-2 cursor-pointer w-6 h-6 sm:top-4 sm:right-4 text-gray-800 dark:text-white">
        ...
        >
          ...
          <Dropdown.Item
            onClick={() => handleStatusChange('job_offer', interview)}
          >
          ...
          <Dropdown.Item
            onClick={() => handleStatusChange('hired', interview)}
          >
          ...
          <Dropdown.Item
            onClick={() => handleStatusChange('rejected', interview)}
          >
    ...
    <div className='flex flex-col lg:flex-row gap-4 justify-center h-full font-sans font-medium m-10'>
      <div className='flex flex-col h-[80vh] min-h-5/6 bg-white rounded-xl lg:w-1/4 overflow-y-scroll'>
        ...
        <div className='bg-white p-4 m-2'>
          {firstInterview?.map(renderInterviewCard)}
        </div>
      </div>
      <div className='flex flex-col h-[80vh] min-h-5/6 bg-white rounded-xl lg:w-1/4 overflow-y-scroll'>
        <span className='bg-gradient-to-r from-cyan-400 via-cyan-500 to-cyan-600 hover:bg-gradient-to-br py-4 text-center text-white rounded-xl'>
          Job Offer
        </span>
        <div className='bg-white p-4 m-2'>
          {jobOffer?.map(renderInterviewCard)}
        </div>
      </div>
      <div className='flex flex-col h-[80vh] min-h-5/6 bg-white rounded-xl lg:w-1/4 overflow-y-scroll'>
        <span className='bg-gradient-to-r from-green-400 via-green-500 to-green-600 hover:bg-gradient-to-br py-4 text-center text-white rounded-xl'>
          Hired
        </span>
        <div className='bg-white p-4 m-2'>
          {hired?.map(renderInterviewCard)}
        </div>
      </div>
      <div className='flex flex-col h-[80vh] min-h-5/6 bg-white rounded-xl lg:w-1/4 overflow-y-scroll'>
        <span className='bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br py-4 text-center text-white rounded-xl'>
          Rejected
        </span>
        <div className='bg-white p-4 m-2'>
          {rejected?.map(renderInterviewCard)}
        </div>
      </div>
    </div>
  );
