...
const Planet = () => {
  ...
  useEffect(() => {
    const fetchPlanets = async () => {
      try {
        const response = await axios.get("http://localhost:3001/planet") // Adjust the API endpoint as per your backend
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (!id) {
        const response = await axios.post("http://localhost:3001/planet", {
          name,
          type,
          isDeleted,
        ...
        const response = await axios.put(`http://localhost:3001/planet/${id}`, {
          name,
          type,
          isDeleted,
        }) // Adjust the API endpoint as per your backend
  ...
  const handleEditPlanet = planet => {
    ...
    setName(planet.name)
    setType(planet.type)
    setIsDeleted(planet.isDeleted)
  }

  // Handle planet delete click
  const handleDeletePlanet = async planetId => {
    try {
      await axios.delete(`http://localhost:3001/planet/${planetId}`) // Adjust the API endpoint as per your backend
          ...
          <tbody className="text-gray-600 text-sm">
            {planets.map(planet => (
              <tr key={planet.id} className="border-b hover:bg-gray-100">
                ...
                <td className="py-4 px-6 text-center">{planet.type}</td>
